...
.mtxt{font-size:.55rem; color:var(--sub); text-align:center; margin-top:2px; text-transform:uppercase; letter-spacing:.04em;}
...
<div class="page">
  ...
  <div class="top">
    <div class="brand">
      ...
      <div><h1>Programmation Horaire</h1><p>4 Relais · richardv.local</p></div>
    </div>
...
rowsEl.innerHTML = RELAYS.map(r => `
  <div class="row" id="${r.id}-row" style="--accent:${r.color}; --accent-rgb:${r.rgb};">
    <div class="row-top">
      ...
      <span id="${r.id}relay-status" class="badge und">---</span>
      <div>
        <label class="toggle">
...
async function update() {
  try {
    ...
    RELAYS.forEach(({id:p}) => {
      ...
      document.getElementById(p + 'mode-text').innerText = isAuto ? "AUTO" : "MANUEL";
      ...
      status.className = "badge " + (etat ? "on" : "off");
    });
